...
(function($) {
        ...
        init : function(options) {
            ...
            return this.each(function(){
                ...
                if(!data){
                            ...
                            for(var col = 0;col < $(this).data("settings").grid.columns;col++){
                                ...
                                $(this).data("frame"+frameNo)
                                    .css({width:$(this).data("settings").width,
                                        ...
                                        'background-position':(-$(this).data("settings").grid.width*col)+"%"+(-$(this).data("settings").grid.height*row)+"%",
                    ...
                    var self = $(this);
                    $('#jsMovie_event_overlay').css({width:$(self).data("settings").width, height:$(self).data("settings").height*2, 'margin-top':'-'+($(self).data("settings").height*2)+"%"});
        ...
        if(this.data("loadStatus") != "loaded" && $(this).data("settings").showPreLoader){
            ...
            this
                ...
                .css({"background-image":"url("+$(this).data("settings").loader.path+")",
                    height:$(this).data("settings").loader.height+"%",
                    width:$(this).data("settings").loader.width+"%",
                    position:'absolute',
                    top:(this.offset().top+(this.height()/2)-($(this).data("settings").loader.height/2))+"%",
                    left:(this.offset().left+(this.width()/2)-($(this).data("settings").loader.width/2))+"%",
            ...
            var loadingInterval = setInterval(function() {
                ...
                self.children(".loaderAnimation").css({'background-position':(-$(self).data("settings").loader.width*(frame%$(self).data("settings").loader.columns))+"% "+(-$(self).data("settings").loader.height*Math.floor(frame/$(self).data("settings").loader.columns))+"%"});
    ...
    function refreshLoaderPosition(){
        this.children(".loaderOverlay").css({top:this.offset().top+"%",left:this.offset().left+"%"});
        this.children(".loaderAnimation").css({top:(this.offset().top+(this.height()/2)-($(this).data("settings").loader.height/2))+"%",left:(this.offset().left+(this.width()/2)-($(this).data("settings").loader.width/2))+"%"});
    }
